...
MAX_PLAYERS = 3
NAME_SIZE = 20
PlayerNames BYTE MAX_PLAYERS * NAME_SIZE DUP(0)
PlayerScores SDWORD MAX_PLAYERS DUP(0)
...
ea6 byte "a)Whale   b)Cat    c)Sparrow",0
...
ea8 byte "a)India   b)Pakistan    c)Russia",0
...
ma3 byte "a)Bajwa   b)Imran Khan    c)Shehbaz Sharif",0
...
ma8 byte "a)Julia Gillard   b)Benazir  c)Jessy Malcolms",0
...
PlayerLoop:
    ...
    call WriteString
    call crlf
    mWrite " A project by Shamveel and Usman "
    call Crlf
    call crlf
    call instruction
...
StoreResult:
    ...
    lea esi, NameOfPlayer
    lea edi, PlayerNames[edi]
    mov ecx, NAME_SIZE
    rep movsb                 
...
LeaderboardProc PROC
    LOCAL tempScore:DWORD
    LOCAL tempName[NAME_SIZE]:BYTE
    
    mov ebx, PlayerScores[0]
    mov ecx, PlayerScores[4]
    cmp ebx, ecx
    jge NoSwap01
    

    mov PlayerScores[0], ecx
    mov PlayerScores[4], ebx
    
   
    lea esi, PlayerNames[0]
    lea edi, tempName
    mov ecx, NAME_SIZE
    rep movsb
    
    lea esi, PlayerNames[NAME_SIZE]
    lea edi, PlayerNames[0]
    mov ecx, NAME_SIZE
    rep movsb
    
    lea esi, tempName
    lea edi, PlayerNames[NAME_SIZE]
    mov ecx, NAME_SIZE
    rep movsb
...
NoSwap01:
    ...
    mov ebx, PlayerScores[0]
    mov ecx, PlayerScores[8]
    cmp ebx, ecx
    jge NoSwap02
    
    mov PlayerScores[0], ecx
    mov PlayerScores[8], ebx
    ...
    lea esi, PlayerNames[2*NAME_SIZE]
    lea edi, PlayerNames[0]
    mov ecx, NAME_SIZE
    rep movsb
    
    lea esi, tempName
    lea edi, PlayerNames[2*NAME_SIZE]
    mov ecx, NAME_SIZE
    rep movsb
    
NoSwap02:

    mov ebx, PlayerScores[4]
    mov ecx, PlayerScores[8]
    cmp ebx, ecx
    jge NoSwap12
   
    mov PlayerScores[4], ecx
    mov PlayerScores[8], ebx
